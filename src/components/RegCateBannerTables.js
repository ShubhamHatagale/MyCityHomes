...
import { faAngleDown, faAngleUp, faArrowDown, faArrowUp, faEdit, faEllipsisH, faExternalLinkAlt, faEye, faSatelliteDish, faTrashAlt } from '@fortawesome/free-solid-svg-icons';
...
// import yoimg from '../pages/uploads/84a2c11bf5cc4d4a3e3cb7cbe6b405ab.png';
...
export const TransactionsTable = () => {
  ...
  const base_url= "/home/shree/Desktop/templates/reactTemplates/volt-react-dashboard-master/src/Server/uploads/";
  ...
  const Register = (e) => {
    ...
    fetch("http://localhost:3002/addCategoryBanners/update", {
      method: "put",
      body: formData,
    })
      .then((res) => res.text())
  ...
  useEffect(() => {
    Axios.get("http://localhost:3002/addCategoryBanners/get").then((response) => {
  ...
  return (
    ...
    <React.Fragment>
      ...
      <Card.Body>
        <h5 className="mb-4">Edit Category Banners</h5>
        <Form>
          <Row>
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          ...
          {
            data.map((curElement) => {
              console.table(curElement.img)
              return (
                <tr>
                  <td>{curElement.id}</td>
                  <td>{curElement.category}</td>
                  <td>{curElement.name}</td>
                  <td><img src={require('/home/shree/Desktop/templates/reactTemplates/volt-react-dashboard-master/src/Server/uploads/'+`${curElement.img}`)} style={{borderRadius:2}} height={60} width={100} /></td>
