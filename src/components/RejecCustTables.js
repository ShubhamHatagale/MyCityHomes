...
export const TransactionsTable = () => {
  ...
  const getData = () =>{
    Axios.get("http://localhost:3002/Rejectedcustomers/get").then((response) => {
 ...
 const rejected = 0
  const ToPendiVeri = (id) =>{
    alert(id)
    Axios.put("http://localhost:3002/reject/update", {
      id: id,
      rejected: rejected,
    }).then((response) => {});
    alert("Sended To Pending Verification");
  }
  return (
    <>
      <React.Fragment>
      ...
      <Table striped bordered hover>
        <thead>
          ...
          <th>Email</th>
          <th>Site details</th>
          <th>Sales executive selected</th>
          <th>Edit</th>
          <th>Pending button</th>
          ...
          {data.map((curElement) => {
            return (
              <tr>
                ...
                <td>
                  <Button onClick={() => edit(curElement.id)}>Edit</Button>
                </td>

                <td>
                  <Button
                    onClick={() => ToPendiVeri(curElement.id)}
                    variant="success"
                    className="m-1"
                  >
