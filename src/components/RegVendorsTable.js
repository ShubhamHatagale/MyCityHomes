...
export const TransactionsTable = () => {
  ...
  const Register = () => {
    Axios.put("http://localhost:3002/Addvendors/update", {
      id: id,
      name: name,
      bname: bname,
      addr: addr,
  ...
  const [data, setDataVendors] = useState([])

  const getData = () => {
    Axios.get("http://localhost:3002/Addvendors/get").then((response) => {
      console.log(response.data)
      setDataVendors(response.data)
      setName(response.data[0].name)
      setdescr(response.data[0].descr)
      setaddr(response.data[0].addr)
      setbname(response.data[0].bname)
      setgst(response.data[0].gst)
      setcont(response.data[0].cont)
  ...
  const edit = (id) => {
    ...
    setdescr(data[id-1].descr)
    setaddr(data[id-1].addr)
    setbname(data[id-1].bname)
    setgst(data[id-1].gst)
    setcont(data[id-1].cont)
        ...
        <Modal as={Modal.Dialog} centered show={showDefault} onHide={handleClose}>
          ...
          <Modal.Body>
            <Card border="light" className="bg-white shadow-sm mb-4">
              <Card.Body>
                <h5 className="mb-4">Edit Vendors</h5>
                <Form>
                  <Row>
            ...
            data.map((curElement) => {
              return (
                <tr>
                  ...
                  <td>{curElement.bname}</td>
                  <td>{curElement.addr}</td>
                  <td>{curElement.gst}</td>
                  <td>{curElement.descr}</td>
                  <td>{curElement.cont}</td>
