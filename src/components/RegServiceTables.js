...
import { Form,InputGroup,Col, Row, Nav, Card, Image, Button, Table, Dropdown, ProgressBar, Pagination, ButtonGroup, Modal } from '@themesberg/react-bootstrap';
...
export const TransactionsTable = () => {
  ...
  const Register = (e) => {
    ...
    fetch("http://localhost:3002/addServices/update", {
      method: "put",
      body: formData,
    })
      .then((res) => res.text())
  ...
  const edit = (id) =>{
    ...
    setshortdesc(data[id-1].shortdesc)
    setlongdesc(data[id-1].longdesc)
    setid(data[id-1].id)
  ...
  useEffect(() => {
    Axios.get("http://localhost:3002/addServices/get").then((response) => {
      console.log(response.data)
      setData(response.data)
      setImg(response.data[0].img)
      setName(response.data[0].name)
      setshortdesc(response.data[0].shortdesc)
      setlongdesc(response.data[0].longdesc)
            ...
            data.map((curElement) => {
              return (
                ...
                <tr>
                  ...
                  <td>{curElement.img}</td>
                  <td>{curElement.mobile}</td>
                  <td>{curElement.shortdesc}</td>
                  <td>{curElement.longdesc}</td>
                  <td><Button onClick={()=>edit(curElement.id)}>Edit</Button></td>
                  <td><BootstrapSwitchButton checked={true} onlabel="Activate" offlabel="Deactivate" onstyle="success" offstyle="danger" width='110' /></td>
